...
public class Scan {
    ...
    public Scan(String input) {
        try {
             br = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(input.getBytes("UTF-8"))));

        } catch(Exception oops) {
            System.err.println("Exception opening stream to string \n");
            oops.printStackTrace();
        }
        // initially, we pretend that we just read a newline.
        // hence linenumber is initialized to 0.
    ...
    // returns com.cc.lc.interpreter.Token
    public Token scan() {
        if( got_eof ) {
